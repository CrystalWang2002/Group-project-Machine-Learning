...
runbagging=function(Y,indice,lag){
  ...
  model=randomForest(cbind(X,dum),y,importance = TRUE, mtr=ncol(Y)-1) #fit the random forest on default settings
...
bagging.rolling.window=function(Y,nprev,indice=1,lag=1){
  ...
  for(i in nprev:1){#NB: backwards FOR loop: going from 180 down to 1
    ...
    lasso=runbagging(Y.window,indice,lag)#call the function to fit the Random Forest and generate h-step forecast
